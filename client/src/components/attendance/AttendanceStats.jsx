...
const AttendanceStats = ({history}) => {
    ...
    const stats = [
        {label: "Days Present", Vaule: totalPresent, icon: CalendarIcon},
        {label: "Late Arrivals", Vaule: totalLate, icon: AlertCircleIcon},
        {label: "Avg. Work Hrs", Vaule: "8.5 Hrs", icon: ClockIcon},
    ]
  return (
        ...
        {stats.map((s) => (
            <div key={s.label} className='card card-hover p-5 sm:p-6 flex items-center gap-4 relative overflow-hidden group'>
                ...
                <div>
                    ...
                    <p className='text-2xl font-medium text-slate-900 tracking-tight'>{s.Vaule}</p>
                </div>
            </div>
        ))}
    </div>
