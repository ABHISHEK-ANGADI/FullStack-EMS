    ...
    <div className='animate-fade-in'>
      <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8'>
        <div>
          ...
          <p className='page-subtitle'>{isAdmin ? "Manage leave applications" : "Your leaves history and requests"}</p>
        </div>
        {!isAdmin && !isDeleted && (
        ...
        <div className='grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-8'>
          {leaveStats.map((s) => (
            <div key={s.label} className='card card-hover p-5 sm:p-6 flex items-center gap-4 relative overflow-hidden group'>
              <div className='absolute left-0 bottom-0 top-0 w-1 rounded-r-full bg-slate-500/70 group-hover:bg-indigo-500/70' />
