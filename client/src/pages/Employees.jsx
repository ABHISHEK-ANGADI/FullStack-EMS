    ...
    <div className='animate-fade-in'>
      ...
      {loading ? (
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5'>
          {filtered.length === 0 ? (
            <p className='col-span-full text-center py-16 text-slate-400 bg-white rounded-2xl border-2xl border border-dashed border-slate-200 '>No employees found</p>
          ) : (
            filtered.map((emp) => <EmployeeCards key={emp.id} employee={emp} onDelete={fetchEmployees} onEdit={(e) => setEditEmployee(e)}/>)
          )}
        </div>
      )}
      ...
      {showCreateModel && (
        <div className='fixed bg-black/40 backdrop-blur-sm inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto' onClick={() => setShowCreateModel(fasle)}>
          ...
          <div className='relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl my-8 animate-fade-in' onClick={(e)=> e.stopPropagation()}>
            <div className='flex items-center justify-between p-6 pb-0'>
              <div>
                ...
                <p className='text-sm text-slate-500 mt-0.5'>Create user account and employee profile</p>
              </div>
